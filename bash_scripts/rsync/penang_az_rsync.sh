#!/bin/sh
#  syncs Amazon Assets to Penang
#  v1.0 - 20140806 - tcosby


LOCK_FILE="/tmp/rsync_penang_az.lock"
LOG_FILE="/usr/schawk/logs/rsync/rsync_penang_az.log"
DAILY_LOG="/usr/schawk/logs/rsync/rsync_penang_az_daily.log"
RSYNC="/usr/bin/rsync"
#RECIPIENTS="thomas.cosby@schawk.com"
RECIPIENTS="thomas.cosby@schawk.com,tholm@schawk.com"

#  source is opus in Minneapolis
SOURCEVOL="/mn_raid2/Amazon/PCBU93007"

#  dest is PengRsync01 at Schawk Penang
DESTVOL="172.17.1.132::rsyncazmpls"
...
if [ -e $LOCK_FILE ]; then
	...
	echo "$TIMESTAMP: lock file detected, exiting" >> $DAILY_LOG
        ...
        mail -s "[Rsync - Mpls Amazon to Penang]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE
...
echo "$MAILTEXT" | mail -s "[Rsync - Mpls Amazon to Penang]" "$RECIPIENTS"
