#!/bin/sh
# syncs _Z ConAgra Assets
#
# Dan B - 02.18.10
# Dan B - 09.02.10 - updated to point at new destination on PengRsync01
# Dan B - 11.20.13 - updated to sync all flattened files, not just Store Brands folder
# Dan B - 01.20.14 - standardized logging across Penang rsyncs
# Dan B - 06.06.14 - revised for GFS
# Thomas - 07.01.14 - excluded PCBU93007/Images/consumer folder to sync to penang

LOCK_FILE="/tmp/rsync_penang_ca.lock"
LOG_FILE="/usr/schawk/logs/rsync/rsync_penang_ca.log"
DAILY_LOG="/usr/schawk/logs/rsync/rsync_penang_ca_daily.log"
RSYNC="/usr/bin/rsync"
#RECIPIENTS="thomas.cosby@schawk.com"
RECIPIENTS="thomas.cosby@schawk.com,tholm@schawk.com"

# source is opus in Minneapolis
SOURCEVOL="/mn_raid2/conagra/PCBU93007"

# dest is PengRsync01 at Schawk Penang
DESTVOL="172.17.1.132::rsynccampls"
...
# perform rsync
$RSYNC -rtvzh --delete --max-delete=100 --timeout=600 --stats --exclude 'Consumer/' --exclude 'Staging/' --exclude '*_P.psd' --exclude '*_p.psd' --exclude '*_P.PSD' --exclude '*_p.PSD' --exclude '*_P?.psd' --exclude '*_p?.psd' --exclude '*_P?.PSD' --exclude '*_p?.PSD' --exclude '*_P??.psd' --exclude '*_p??.psd' --exclude '*_P??.PSD' --exclude '*_p??.PSD' --exclude '*_P???.psd' --exclude '*_p???.psd' --exclude '*_P???.PSD' --exclude '*_p???.PSD' --exclude '.*' "$SOURCEVOL" "$DESTVOL" 2>&1 | tee >> $DAILY_LOG
...
echo "$MAILTEXT" | mail -s "[Rsync - Mpls ConAgra to Penang]" "$RECIPIENTS"
#
#
exit 0
