...
echo "     Start:  " `date`
find /mn_raid*/*/*/Archive\ Staging/*/*/080_QC/* -depth ! -path '*/080_QC/3D Renderings*' ! -path '*/080_QC/Code Verification*' ! -path '*/080_QC/Compare*' ! -path '*/080_QC/PDF for Blue*' -exec rm -Rfv {} \; >> /mn_raid2/productionfiles/Archive_Logs/080_QC/080_QC_$( date +%m%d%y.%H%M%S ).txt
echo "     End:  " `date`;echo
