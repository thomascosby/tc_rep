...
#+ individual names.
###########################################################
#  RSYNC Anthem_BU to Anthem_BU REMOTE (Rsync Mac Pro at Schawk SF)

TIMESTAMP=`date '+%m.%d.%y-%H.%M'`
DAILY_LOG="/Library/Logs/rsync/rsync_remote_anthem_"$TIMESTAMP".log"
RUN_LOG="/Library/Logs/rsync/rsync_remote_anthem.log"
LOCK_FILE="/tmp/rsync_remote_anthem.lock"
...
if [ -f $LOCK_FILE ]; then
	TIMESTAMP=$(date)
        ...
        mail -s "[ASF Remote rsync Anthem Volume - ERROR]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE

SOURCEVOL=/Volumes/3TB_R01A/Anthem_BU/
DESTVOL="10.2.10.26:/Volumes/anthem_sf/Anthem_BU"
...
#  perform for reals rsync
   ...
   echo "*** ERROR IN ANTHEM REMOTE RSYNC - SOURCE PATH NOT FOUND ***" >> $DAILY_LOG
fi
...
echo "$MAILTEXT" | mail -s "[ASF Remote rsync Anthem Volume]" "$RECIPIENTS"

# delete daily logs older than two weeks
find /Library/Logs/rsync/rsync_remote_anthem_*log -mtime +14 -exec rm {} \;
