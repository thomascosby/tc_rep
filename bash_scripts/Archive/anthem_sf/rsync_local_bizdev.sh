...
#+ individual names.
###########################################################
#  RSYNC biz dev to biz dev_BU - locally

TIMESTAMP=`date '+%m.%d.%y-%H.%M'`
DAILY_LOG="/Library/Logs/rsync/rsync_local_biz_dev_"$TIMESTAMP".log"
RUN_LOG="/Library/Logs/rsync/rsync_local_biz_dev.log"
LOCK_FILE="/tmp/rsync_local_biz_dev.lock"
...
if [ -f $LOCK_FILE ]; then
	TIMESTAMP=$(date)
        ...
        mail -s "[ASF local rsync BizDev Volume] - ERROR]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE

SOURCEVOL=/Volumes/10TB_RAID/Business_Development/
DESTVOL=/Volumes/3TB_R01A/Business_Development_BU/
START_TIME=`date`
...
#  perform for reals rsync
   ...
   echo "*** ERROR IN BIZ DEV RSYNC - SOURCE OR DEST PATH NOT FOUND ***" >> $DAILY_LOG
fi
...
echo "$MAILTEXT" | mail -s "[ASF local rsync BizDev Volume]" "$RECIPIENTS"

# delete daily logs older than two weeks
find /Library/Logs/rsync/rsync_local_biz_dev_*log -mtime +14 -exec rm {} \;
