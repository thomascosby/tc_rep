#!/bin/sh
# syncs Dial/Assets/Corrugated and Dial/Dielines/Corrugated from Mpls to Penang
#
# Dan B - 05.09.12
# Dan B - 01.20.14 - standardized logging across Penang rsyncs

LOCK_FILE="/tmp/rsync_penang_dial.lock"
LOG_FILE="/usr/schawk/logs/rsync/rsync_penang_dial.log"
DAILY_LOG="/usr/schawk/logs/rsync/rsync_penang_dial_daily.log"
RSYNC="/usr/bin/rsync"
#RECIPIENTS="dberks@schawk.com,tholm@schawk.com"
RECIPIENTS="dberks@schawk.com"

# source is opus in Minneapolis
SOURCEVOL="/mn_raid2/dial"

# dest is PengRsync01 at Schawk Penang
DESTVOL="172.17.1.132::rsyncdialmpls"
...
if [ -e $LOCK_FILE ]; then
	...
	echo "$TIMESTAMP: lock file detected, exiting" >> $DAILY_LOG
        ...
        mail -s "[Rsync - Mpls Dial to Penang]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE
...
# perform rsync
echo "###  Assets/Corrugated  ###" >> $DAILY_LOG
$RSYNC -rtvzh --timeout=600 --delete --max-delete=100 --stats --exclude '.*' $SOURCEVOL/Assets/Corrugated "$DESTVOL/Assets/" 2>&1 | tee >> $DAILY_LOG
echo "" >> $DAILY_LOG

echo "###  Dielines/Corrugated  ###" >> $DAILY_LOG
$RSYNC -rtvzh --timeout=600 --delete --max-delete=50 --stats --exclude '.*' $SOURCEVOL/Dielines/Corrugated "$DESTVOL/Dielines/" 2>&1 | tee >> $DAILY_LOG
...
echo "$MAILTEXT" | mail -s "[Rsync - Mpls Dial to Penang]" "$RECIPIENTS"
