...
if [ -e $LOCK_FILE ]; then
	...
	echo "$TIMESTAMP: lock file detected, exiting" >> $DAILY_LOG
        ...
        mail -s "[Rsync - Mpls Safeway to Penang]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE
...
$RSYNC -rtvzh --timeout=600 --delete --max-delete=100 --stats --exclude 'Staging/' --exclude '*_P.psd' --exclude '*_p.psd' --exclude '*_P.PSD' --exclude '*_p.PSD' --exclude '*_P?.psd' --exclude '*_p?.psd' --exclude '*_P?.PSD' --exclude '*_p?.PSD' --exclude '*_P??.psd' --exclude '*_p??.psd' --exclude '*_P??.PSD' --exclude '*_p??.PSD' --exclude '*_P???.psd' --exclude '*_p???.psd' --exclude '*_P???.PSD' --exclude '*_p???.PSD' --exclude '.*' "$SOURCEVOL" "$DESTVOL" 2>&1 | tee >> $DAILY_LOG
...
echo "$MAILTEXT" | mail -s "[Rsync - Mpls Safeway to Penang]" "$RECIPIENTS"
#
#
exit 0
