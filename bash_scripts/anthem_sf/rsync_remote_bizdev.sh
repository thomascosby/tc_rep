...
#+ individual names.
###########################################################
#  RSYNC Business_Development_BU to BizDev_BU Remote (Rsync Mac Pro at Schawk SF)

TIMESTAMP=`date '+%m.%d.%y-%H.%M'`
DAILY_LOG="/Library/Logs/rsync/rsync_remote_bizdev_"$TIMESTAMP".log"
RUN_LOG="/Library/Logs/rsync/rsync_remote_bizdev.log"
LOCK_FILE="/tmp/rsync_remote_bizdev.lock"
...
if [ -f $LOCK_FILE ]; then
	TIMESTAMP=$(date)
        ...
        mail -s "[ASF Remote rsync BizDev Volume] - ERROR]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE

SOURCEVOL=/Volumes/3TB_R01A/Business_Development_BU/
DESTVOL="10.2.10.26:/Volumes/anthem_sf/BizDev_BU"
...
#  perform for reals rsync
   ...
   echo "*** ERROR IN BIZ DEV REMOTE RSYNC - SOURCE PATH NOT FOUND ***" >> $DAILY_LOG
fi
...
echo "$MAILTEXT" | mail -s "[ASF Remote rsync BizDev Volume]" "$RECIPIENTS"

# delete daily logs older than two weeks
find /Library/Logs/rsync/rsync_remote_bizdev_*log -mtime +14 -exec rm {} \;
