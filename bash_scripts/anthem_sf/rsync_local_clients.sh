...
#+ individual names.
##########################################################################################
#  RSYNC Clients to Clients_BU - Locally

TIMESTAMP=`date '+%m.%d.%y-%H.%M'`
DAILY_LOG="/Library/Logs/rsync/rsync_local_clients_"$TIMESTAMP".log"
RUN_LOG="/Library/Logs/rsync/rsync_local_clients.log"
LOCK_FILE="/tmp/rsync_local_clients.lock"
...
if [ -f $LOCK_FILE ]; then
	TIMESTAMP=$(date)
        ...
        mail -s "[ASF Local rsync Clients Volume] - ERROR]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE

SOURCEVOL=/Volumes/15TB_RAID/Clients/
DESTVOL=/Volumes/3TB_R01B/Clients_BU/
START_TIME=`date`
...
#  perform for reals rsync
   ...
   echo "*** ERROR IN Clients RSYNC - SOURCE OR DEST PATH NOT FOUND ***" >> $DAILY_LOG
fi
...
echo "$MAILTEXT" | mail -s "[ASF Local rsync Clients Volume]" "$RECIPIENTS"

# delete daily logs older than two weeks
find /Library/Logs/rsync/rsync_local_clients_*log -mtime +14 -exec rm {} \;
