...
#+ individual names.
###########################################################
#  RSYNC Operations to Operations_BU

TIMESTAMP=`date '+%m.%d.%y-%H.%M'`
DAILY_LOG="/Library/Logs/rsync/rsync_local_operations_"$TIMESTAMP".log"
RUN_LOG="/Library/Logs/rsync/rsync_local_operations.log"
LOCK_FILE="/tmp/rsync_local_operations.lock"
...
if [ -f $LOCK_FILE ]; then
	TIMESTAMP=$(date)
        ...
        mail -s "[ASF Local rsync Ops Volume] - ERROR]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE

SOURCEVOL=/Volumes/10TB_RAID/Operations/
DESTVOL=/Volumes/3TB_R01A/Operations_BU/
START_TIME=`date`
...
#  perform for reals rsync
   ...
   echo "*** ERROR IN OPERATIONS RSYNC - SOURCE OR DEST PATH NOT FOUND ***" >> $DAILY_LOG
fi
...
echo "$MAILTEXT" | mail -s "[ASF Local rsync Ops Volume]" "$RECIPIENTS"

# delete daily logs older than two weeks
find /Library/Logs/rsync/rsync_local_operations_*log -mtime +14 -exec rm {} \;
